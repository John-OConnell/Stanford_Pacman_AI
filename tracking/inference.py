...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        # Cover the case of an empty distribution and the case where the total
        # value of the distribution is zero
        if not self or self.total() == 0:
            return
        else:
            # Get total of all values and normalize each value in distribution
            selfTotal = self.total()
            for key in self:
                self[key] = self[key] / selfTotal
            return
        "*** END YOUR CODE HERE ***"
    ...
    def sample(self):
        ...
        # Handle the case where the distribution has not been normalized
        if self.total() != 1:
            selfTotal = self.total()
            for key in self:
                numTimes = int((self[key] / selfTotal) * 100)
                temp = [key] * numTimes
                weightedDis.extend(temp)

        # Handle the case wehre the distribution has been normalized
        else:
            for key in self:
                numTimes = int(self[key] * 100)
                temp = [key] * numTimes
                weightedDis.extend(temp)

        # Pick randomly from newly created weighted distribution
        sample = random.choice(weightedDis)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Find new bliefs based on observation probability
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in positions:
            # Get distribution over new positions for the ghost
            newPosDist = self.getPositionDistribution(gameState, pos)
            # Loop through new positions and update new belief distribution
            for newPos in newPosDist:
                newBeliefs[newPos] += newPosDist[newPos] * self.beliefs[pos]

        # Replace old belief distribution with new
